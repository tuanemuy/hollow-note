...
- **A fragment that reads more than once has to place every read after the first**, on one of two sides. Fold it into the same `try` when what it returns is the body of the screen (`WorkspaceMembersPanel`'s pending invitations), or swallow it — `.catch(() => null)` — when it only adds a decoration (`PublicWorkspacePage`'s viewer-role banner, which the page's content does not depend on). Leaving a second read outside the classification splits the terminal state by viewer: the same instant of the same screen renders "not found" for the reader whose first call lost the race and the route's `errorComponent` for the one whose second call did. An extra read *inside* a client island is not covered by this — it owns its own error UI.
