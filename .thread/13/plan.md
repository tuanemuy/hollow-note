...
| AC-8 | 本番ビルド（`pnpm build && pnpm start`）で `/notes` → `/notes/:id` → ブラウザ戻る、としたとき 2 回目の `/notes` 表示は **`_serverFn` 1 本**（`shouldReload` によりガードを載せた loader が必ず再実行される）。その再取得は**背景で走る**ので**遷移は即座に settle** し、**前回の一覧が表示されたまま置き換わる（スケルトンに戻らない）**。**観測点は `NoteListSkeleton` の再表示**であり、要求本数だけを数えて合格にしない。**「スケルトンに戻らない」の根拠は「背景で走るから」ではない** — 背景枝の `updateMatch` は `loaderData` を未解決の断片 promise ごと差し替え、その更新は `useSyncExternalStore` 経由で SyncLane に届くので、消費側が何もしなければマウント済みの `<Suspense>` はフォールバックへ戻る。戻らないのは **`Deferred` が `use(useDeferredValue(promise))` で差し替えを deferred lane に載せている**からである（ADR-005）。**実測で成立を確認済み**: 本番ビルドの戻る操作 4 回とも `NoteListSkeleton` の出現 0 件（`Deferred` 修正前は戻る操作の 21ms 後に 1 件出現）。**「1 本」はステップ 1 が入って初めて成立する** — ステップ 3 だけを入れた状態では root の `beforeLoad` が `staleTime` を見ずに毎ロード `loadAppContext` を撃つので 2 本になる。**`staleReloadMode: "blocking"` は `/settings` レイアウトにだけ置き、`/notes` 系には置かない** — blocking が await するのは loader（ガード 1 往復）だけで、commit 時点の断片 promise は未解決のままなのでスケルトン対策にならず、URL 確定が遅れるだけである（ADR-005 の却下案） | 既存の断片ストリーミング契約の維持 / ADR-003 / ADR-005 | 1, 3 |
...
- **トップ `/` の `sessionUserFn` 往復**。サインイン済みを `/notes` へ飛ばすだけのガードで、統合できる断片を持たない。root の `loadAppContext` が消えれば `/` は理論上 1 往復（`sessionUserFn` のみ）が下限になる。`/` は基本フルロードで入る画面なので本 Issue の「ナビゲーションごとの 3 往復」には当たらない。**除外の根拠を事後に裏づけるため、`/` へのクライアント遷移で `_serverFn` の本数を実測して記録する**（AC ではなく記録項目。AC-4 の公開ルート一覧に `/` が入らない理由もこれ）。**観測経路は「未サインインで `/signin` → `/`」（`AuthLayout` のロゴリンク）に固定する** — サインイン済みのシェル（`components/layout/AppShell`）に `/` へのリンクは無く（`to="/"` を持つのは `PublicShell` / `AuthLayout` / `OAuthCallbackPanel` だけ）、仮に起こしても `routes/index.tsx:12-17` の `beforeLoad` がサインイン済みを `/notes` へ redirect するため `sessionUserFn` のあとに `/notes` の loader がもう 1 本走り、記録値が崩れる。**`/` は loader を持たず `beforeLoad` だけのルートで、`executeBeforeLoad` にキャッシュ判定・dedupe が無い。** そのため通常どおりマウスでクリックして測ると、ナビゲーション本体のガード 1 本に加え、**クリック操作自体が 50ms 後にもう一度予約する intent preload**（`useLinkProps` の `onMouseEnter`/`onFocus`）が別の `loadMatches` パスから直列に走り、実測は **2 本 / 直列 2 段**になる（`main` は同じ経路に `loadAppContext` も乗るため 4 本）。「下限 1 往復」という主張自体は成立するが、それを直接示すには**リンクを先にフォーカスして preload を出し切ってから、新たな `mouseenter`/`focus` を伴わない形で起動する**（Enter キー、またはフォーカス済み要素への `element.click()`）測り方が要る（詳細は `.thread/13/manual-test/edge-4-investigation.md`）。
...
- **ホバー中のクリックはガードの再実行を 1 回落とす（loader を持つルート限定）。** preload が in-flight のまま同じリンクをクリックすると、`loadRouteMatch`（`src/load-matches.ts:893-904`）が `prevMatch._nonReactive.loaderPromise` を見て `status === 'success' && !sync && !prevMatch.preload && shouldReloadInBackground` で早期 return するため、その遷移ではガードが再判定されない。ガード自体は preload 側の背景ロードで走り、redirect も同 843-847 が `router.navigate` で拾うので挙動は壊れないが、**AC の本数は 1 本ずれる**。受け入れ基準の前置きで「preload が完了してからクリックする」を計測条件に固定してあるのはこのため。この dedupe は **loader 側にしか無い**ので、AC-1〜AC-4 の対象（すべて loader を持つ）にのみ当てはまる。`beforeLoad` だけで loader を持たないルート（`/`）は `preBeforeLoadSetup` が in-flight の `beforeLoadPromise` を待ってから必ず再実行するため、in-flight クリックでも本数は減らず、逆にクリックが誘発する intent preload の分だけ 1 本多く出る（`.thread/13/manual-test/edge-4-investigation.md`）。
- **`/settings` のガードを loader へ移すと、未サインインでも子の断片 loader が並列に発火して 401 を 1 本無駄に打つ。** `loadMatches` は走査順（親が index 0）で redirect を先に throw するので遷移としては正しく `/signin` へ行くが、Network には 401 が 1 本残る。**これはナビゲーションに限らない** — 失効後にタブへ**ホバーしただけ**でも同じ並列が起き（レイアウト match はアクティブなので `cause: "stay"` → `shouldReload` 真）、子断片の 401 が 1 本飛んで cached match が `status: "error"` で残る。**退行ではない**（クリック時は `status !== "success"` で再実行され、レイアウトの redirect が遷移を奪う）ので、実測でこれを退行として記録しないこと。
...
2. 次の遷移を **アプリ内リンク／ブラウザ戻る**（フルリロードではない）で行い、各遷移ごとに要求本数と `Start Time` を記録する。**リンクはホバーしてから preload の `_serverFn` が完了するのを待ってクリックし、preload 分と クリック以降の分を欄を分けて数える**（前置きの「preload 要求の扱い」を参照。`defaultPreload` は外さない）。遷移が終わるたびに **HAR を書き出して `.thread/13/` に保存**する。
   ...
   - **未サインインで** `/signin` → `/`（`AuthLayout` のロゴリンク。スコープ外の記録項目: マウスクリック計測で **2 本 / 2 段**。`main` は 4 本）。サインイン済みの `/notes` → `/` では測らない — `AppShell` に `/` へのリンクが無く、`routes/index.tsx` の `beforeLoad` が `/notes` へ redirect し返すので本数が崩れる
...
9. 本番ビルド（AC-8）: `pnpm build && pnpm start` で `/notes` → `/notes/:id` → 戻る。要求 1 本・**戻る操作の直後に `NoteListSkeleton` が再表示されないこと**・一覧が表示されたまま置き換わることを見る。スケルトンの有無が観測の主眼で、要求本数だけでは合格を決めない（根拠は背景枝ではなく `Deferred` の deferred lane 化 — ADR-005）。あわせて `Deferred` は `/settings` の 3 断片からも使われるので、初回マウントのスケルトンとミューテーション後の `router.invalidate()` が壊れていないことを回帰として見る（testing.md 確認項目 19）。
