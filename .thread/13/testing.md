...
**手順 6 の「リンクをクリックする」という操作自体が、50ms 後にもう 1 本の intent preload を予約する**（`useLinkProps` が `onMouseEnter` / `onFocus` の両方を preload に繋いでおり、クリック直前の `mouseenter` / `focus` がタイマーを張るため）。これが本数に乗るのは **loader を持たず `beforeLoad` だけのルート**に限る — loader を持つルート（`/notes` 系・`/settings` 系）は `loadRouteMatch` に dedupe があるため 2 回目は握り潰され、本数に影響しない。`beforeLoad` だけのルート（本 Issue の対象では `/`。エッジケース 4 参照）を測るときは、この分が「クリック以降」に混ざることを踏まえて本数を読む。
...
- **期待結果:**
  ...
  - 手順 5: `/signin` へ飛ばず `/settings/danger` に留まる。ticket は完了到達時に破棄される設計なので、リロードが「アカウントを削除しています」の表示中に間に合った場合に限り `sessionStorage` の ticket から進捗表示が復帰する。間に合わなかった場合は空の削除フォームに戻るのが正しい挙動。
- **確認ポイント:**
  - 手順 4 の時点で**セッションは既に失効している**（受理と同時にサインアウトされる）ので、手順 5 は「未サインインで `/settings/danger` を開く」経路そのもの。ここで `/signin` に飛んだら `SIGNED_OUT_PATH` の分岐が `loader` 移行で落ちている。合否の中心はここに置き、進捗表示の復帰有無だけでは判定しない。
  - ticket は完了到達時に `forgetTicket()` で破棄されるため、完了表示（「アカウントを削除しました」）に到達した後のリロードでその表示に戻ることはない。in-memory バックエンドでは受理〜完了が数十 ms しかなく、実測でもこの窓を人手のリロードで捕捉できないのが普通（=空の削除フォームに戻ることが多い）。
...
### 2. preload が in-flight のままクリックすると本数が 1 本少なく出る（loader を持つルート限定）
...
- **確認ポイント:**
  - これは**不具合ではない**。この形で測った数値を AC の実測値として記録しないこと。記録済みの HAR がこの状態でないかは、preload 欄とクリック欄の要求名が重複していないかで見分ける。
  - **この「1 本少なく出る」は `loadRouteMatch` の dedupe を持つ loader ベースのルート限定**（AC-1〜AC-4 の対象はすべて loader を持つ）。`beforeLoad` だけで loader を持たないルート（`/`。エッジケース 4 参照）では逆に働く — `preBeforeLoadSetup` が in-flight の `beforeLoadPromise` を待ってから**必ず**再実行するため、in-flight クリックでも本数は減らず、クリックが誘発する intent preload の分だけ**むしろ 1 本多く**出る。
...
### 4. `/` へのクライアント遷移が 2 本 / 2 段になる（マウスクリック計測。スコープ外の記録項目）

- **検証手段:** browser
- **目的:** 「`loadAppContext` が消えたあと `/` の下限は 1 往復（`sessionUserFn` のみ）」という除外の根拠を事後に裏づける。ただし `/` は loader を持たず `beforeLoad` だけのルートで dedupe が無いため、マウスでクリックする通常の計測では **クリック自体が誘発する intent preload がもう 1 本乗る**ことをあわせて記録する（`.thread/13/manual-test/edge-4-investigation.md`）
- **手順:**
  1. **未サインイン**で `/signin` を開く。
  2. 画面上部のロゴ（`AuthLayout` の「Hollow のトップへ」）を対象に、計測手順 1〜8 を実行する。
- **期待結果:** クリック以降の `_serverFn` が **2 本 / 直列 2 段**（内訳: ナビゲーション本体のガード 1 本 + クリック操作が誘発する intent preload 1 本。どちらも `sessionUserFn`）。`main` では `loadAppContext` + `sessionUserFn` がパスごとに乗るため **4 本**。
- **確認ポイント:**
  - **サインイン済みの `/notes` → `/` では測らない。** `AppShell` に `/` へのリンクは無く、仮に起こしても `routes/index.tsx` の `beforeLoad` がサインイン済みを `/notes` へ redirect し返すので、`/notes` の loader がもう 1 本走って本数がずれる。
  - **「下限 1 往復」そのものを直接見たい場合**は、通常の手順 1〜8 ではなく次の測り方に切り替える: リンクを**先にフォーカスして preload を出し切り**（`focus()` または Tab キー、Network が静止するまで待つ）、そのあと**新たな `mouseenter` / `focus` を伴わない形でナビゲーションだけを起動する**（キーボードの Enter、またはフォーカス済み要素への `element.click()`）。この形なら `sessionUserFn` は **1 本**になる。これは AC ではなく記録項目。
