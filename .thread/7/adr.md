...
## ADR-123: `MEDIA_SVG_MAX_BYTES` の導出を正典から降ろし、約束は境界翻訳の網羅で担保する

### Context

ADR-115 は「128 KB の根拠を入力の形から `HtmlProcessorLimit.maxExpansionFactor` へ置き換える」と決め、`131,072 × 4 = 524,288 < 800,000` を「上限が到達可能であることの唯一の根拠」と書いた。**これも偽である。**

レビュー 008 backend-storage [B-001] の実測（本作業で再現）: 単一引用符の属性値に生の `"` を 131,072 バイトちょうど詰めた SVG は `ensureAcceptable` を通り、`htmlProcessor.process` が **786,073 バイト**を返す。524,288 の 1.5 倍で、本文の 800,000 まで残り 1.74%。原因は膨張のメーターが `attribute.value.length` を**エスケープ前**で課金する一方（`htmlProcessor.ts` の `createElement`）、直列化が `"` を `&quot;` に開くこと — 1 課金・6 出力である。

さらに本作業の実測で、その 6 倍が**再構成で掛け算されること**が分かった。`<!-->` に隠した `<table>` の後ろに `<b title='"×20,000'>` を置き `<tr>` を 6 行並べた **20 KB** の SVG は、受理判定を通り、メーターにも掛からず、`NoteHtml.create` が `NOTE_CONTENT_TOO_LARGE` で落ちる。つまり [W-001] が「到達不能」と見ていた経路は**今日すでに到達可能**で、`storeMedia` は Note の語彙をそのまま外へ漏らしていた。

導出が偽だった回数はこれで 4 回になる（形の論証で 3 回、上限どうしの不等式で 1 回）。誤っていたのは値ではなく、**約束を導出で支える形**そのものである。

### Decision

**値も上限もメーターも動かさない。「導出そのもの」を降ろす。**

1. `processSvg` の境界翻訳を `NoteErrorCode.ContentTooLarge` へ**広げる**。`HtmlProcessor.process` が Note の語彙で投げうるのはこの 2 コードで尽きる — 資源の 3 つの上限（メーター・入れ子の走査・CSS の予算）はすべて `NOTE_HTML_TOO_COMPLEX` の 1 コードに落ち、結果の測り直しは `NoteHtml.create` / `PlainTextContent.create` / `Excerpt.create` の 3 つだけで、これが `process` が構築する Note の値のすべてである。翻訳先は両方 `FileTooLarge`（どちらも長さの上限であり、費用を払えば手順 4 が返す答えと同じ）
2. `MEDIA_SVG_MAX_BYTES` の JSDoc から導出を落とし、「編集者が挿入してよい大きさの方針値であって、何もこれに依存していない」と書き直す。4 度の反証（86 倍 / 180 倍 / 786,073 / 20 KB で 800,000 超）は**導出が成り立たない証拠**として残す
3. 「Note の語彙が Storage の外へ漏れない」は、gate の完全性でも上限どうしの不等式でもなく、**この 2 コードを覆う `catch` 1 つ**が担保する

### Alternatives

- **`MEDIA_SVG_MAX_BYTES` を下げる**（例えば 800,000 ÷ 6 = 133 KB より下へ）: 6 倍は 1 属性の話で、再構成が掛かると 20 KB でも超える。下げる先が無い
- **メーターをエスケープ後の長さで課金する**: `spec/adr/013` の「解析しながら測る」を壊す（直列化前に直列化長を正確に知ることはできない）。しかも 5 版目の導出になる
- **`NoteHtml` の上限を上げる**: `spec/adr/017` の行の予算に触れる。Storage の都合で本文の設計値を動かす話になる

### Consequences

- 良い点: 「128 KB を上げる / 本文の上限を下げる / 許可リストに整形要素を足す / parse5 のエスケープ集合が広がる」のどれも、約束を静かに壊さなくなる。破れるのは翻訳を狭めたときだけで、そのときは TC-storage-274 が red になる
- 良い点: 実在した漏れ（20 KB の SVG が `NOTE_CONTENT_TOO_LARGE` を返していた）が閉じる。UI では「本文が長すぎる」というノート本文向けの案内が出ていた
- トレードオフ: `FileTooLarge` が 20 KB のファイルにも出る。文言「ファイルが大きすぎます」は厳密には「この大きさでは扱えない」だが、利用者の取れる行動（小さく・単純にする）は一致する（ADR-115 と同じ判断）
- 残件: この 2 コードが `process` の全体であることは `domain/note/ports/htmlProcessor.ts` の契約には書かれていない。ポート側に「Note の語彙で投げうるコードの集合」を宣言し、`storeMedia` がそれを引く形にすれば列挙の同期も型で持てる。今回は担当外（`domain/note/` は別単位）なので JSDoc に列挙だけ置いた
- 変異スポットチェック: `NOTE_LIMIT_CODES` から `ContentTooLarge` を落とすと TC-storage-274 が red（`NOTE_CONTENT_TOO_LARGE` が漏れる）、手順 4 の測り直しを `storedSize` から `accepted.size` に戻すと TC-storage-273 と TC-storage-252 が red

---

## ADR-124: 1 turn の上限はページではなく「その turn が触るノート数」に掛ける

### Context

ADR-107 は「持ち回る `StuckPurge` はページの上に載るので、回収中の turn は 480 を超えうる。これは実上限 1,000 との差で吸収する」と書いた。レビュー 008 backend-tag [W-001] が指摘したとおり、**その差はもう埋まっている**。`SCOPE_TASK_TICK_LIMIT = 100` により 1 invocation は `note.ownerPurgeContinued` と `note.trashExpiryContinued` を同居させうるので、同居だけで 80 notes × 12 = 960。残りは 40 文（purge 3 件分）しかない。

しかも持ち回り集合に上限が無い。`carried` に入るのは `isOutOfReach` のノート — local delete が済んで route が resolve しなくなったもの — だけなので、`listByOwner` には二度と現れない。`targets = batchSize + N` で、`removeForPurge` / `finishPurge` が continuously に失敗する障害では **`N` が 1 turn あたり最大 40 ずつ増え、減らすものが何も無い**。継続 payload も同じだけ膨らむ。

### Decision

**ページ側を持ち回りで削る。** `listByOwner` の `limit` を `Math.max(0, batchSize - carried.length)` にする。`batchSize` は「1 turn が触るノート数」の上限であって「ページの幅」ではない、という読みに直す。

- 持ち回りが `batchSize` を埋め切った turn はページを 0 件で読む。`count` は `limit` と独立に総数を返す（memory は `matched.length`、Cloudflare は別 statement の `COUNT(*)`）ので、継続の判定材料は失われない
- 持ち回りを捨てる方向は取れない。その id はこの turn のメモリ以外どこにも無い

### Consequences

- 良い点: 1 turn が 40 notes = 480 statement で閉じる。正典の算術が「同居 960 ＋ 持ち回りはその 40 の内側」で閉じられるようになる
- 良い点: 継続 payload の大きさも同じ上限で閉じる
- トレードオフ: 持ち回りが溜まっている scope では列挙の前進が遅くなる（極端には止まる）。ただし持ち回りが溜まる状況は global が壊れている状況そのもので、そこでページを進めても purge は成功しない
- 残件: 同居側（`SCOPE_TASK_TICK_LIMIT` が purge 系 task を 2 本通しうる）は塞いでいない。ADR-114 が挙げた「1 turn が訪問する purge 系 task を 1 本に絞る」がその手当てで、本作業の担当外
- 変異スポットチェック: `limit` を `batchSize` に戻すと TC-note-827 / TC-note-789 が red、`- carried.length + 1` にすると TC-note-074 / 076 / 079 / 788 / 789 / 827 の 8 ケースが red

---

## ADR-128: 決定的な業務拒否の集合は「一時的な失敗」の補集合として閉じる

### Context

編集画面（P-12）は版を進めうる往復が投げたものを 1 つの `classify` で状態へ写している。`rejected`（再試行のボタンも端末への退避も出さない側）は 4 コードの列挙で、`NOTE_CONTENT_TOO_LARGE` / `NOTE_INVALID_TITLE` / `NOTE_INVALID_STYLE_MODE` / `INVALID_INPUT` だけが当たっていた。

列挙は実際に届く拒否を尽くしていなかった。`updateNoteBody` は本文を必ず `htmlProcessor.process()` に通すので資源上限の `NOTE_HTML_TOO_COMPLEX` が届き、版の復元は保持 20 版から溢れた版で `REVISION_NOT_FOUND` を返し、`applyTextNodeEdits` は `NOTE_CANNOT_CAPTURE_EMPTY_CONTENT` を投げる。どれも `default` 枝に落ちて `failed` になり、(1) 成功しえない「再試行」が出る、(2) 800 KB まで許される本文が毎回 `localStorage` へ書かれる、(3) 「内容はこの端末に退避したので、次に開いたときに復元できます」と告げて直せば通ることを伝えない、が同時に起きていた。`presentation/errorDisplay.ts` が `NOTE_HTML_TOO_COMPLEX` について「共通文言の『もう一度お試しください』は実行不能な助言になる」と書いているのに、その文言を包む Alert のほうが「もう一度お試しください」の枠だった。

列挙を 3 つ足すだけでは同じことが次のコードで起きる。拒否はドメインが増やすもので、画面はそれを知らないまま既定の枝に落とす。

### Decision

集合を**逆向きに閉じる**。列挙するのは「往復そのものが成立しなかった」`kind` のほうにし、それ以外はすべて決定的な業務拒否として扱う。

- `TRANSIENT_ERROR_KINDS = { system, unknown, unauthorized }` — 通信断・サーバー側の障害・セッション切れ。あとで同じ内容が通るので、再試行と端末への退避に値する唯一の集合
- 面ごと凍らせる 4 コード（`NOTE_NOT_FOUND` / `NOTE_ACCESS_DENIED` / `NOTE_IS_TRASHED` / `WORKSPACE_INSUFFICIENT_ROLE`）と `OPTIMISTIC_LOCK_FAILURE` / `NOTE_LOCKED_BY_JOB` は先に名前で拾う（それぞれ専用の状態を持つ）
- 残りは `rejected`。列挙していないコードは既定でこちらへ落ちる
- 判定は `NoteEditorIsland` の外へ出し、モジュールスコープの純関数 `classifySaveFailure(error, retry)` にする

`spec/pages/index.md` の P-12 状態表もこの向きで書き直した（「保存できません（内容の拒否）」の行が補集合の規則を持ち、括弧内は例示になる）。

### Consequences

- 良い点: ドメインが拒否コードを増やしても画面が取りこぼさない。誤るとしても「一時的な失敗を決定的な拒否として出す」方向にしかならず、その場合でも内容を直す打鍵で状態が降りて自動保存が再開する（`markDirty` が `rejected` を打鍵で降ろす）
- 良い点: 辞書（`errorDisplay`）と、その文言を包む枠が同じエラーについて同じことを言うようになった
- 良い点: 判定がモジュールスコープの純関数になったので、`spec/pages/index.md` の集合との一致を単体で固定できる形になった
- トレードオフ: 「待てば通る」性質の `conflict` 系コード（移動サガの 4 コードなど）がこの画面へ届くようになると `rejected` に落ちる。文言そのものは辞書が正しく出し、打鍵で降りて再開するので実害は無いが、案内の末尾（「内容を直すと自動保存を再開します」）は最適ではない
- 変異スポットチェック: `TRANSIENT_ERROR_KINDS` に `business` を足すと、上限超えの本文が再び `localStorage` へ書かれる

---

## ADR-129: `failed` は落ちた往復を値で持ち、「再試行」はそれを送り直す

### Context

`classify` は 4 つの `catch`（本文の保存・モード切替・競合の解決・版の復元）で共有されているのに、`failed` Alert の「再試行」は無条件に `commit(true)`（本文の保存）だった。版の復元が通信エラーで落ちたあと「再試行」を押すと、復元ではなく保存が走る。Alert のコメントは「モード切替・競合の解決・版の復元が通信エラーで落ちたときは退避が無いまま `failed` になる — そこは既存のノートなので、再試行こそが逃げ道である」と正しく書いていたが、その「再試行」が指す操作が失敗した操作と一致していなかった。

### Decision

`SaveStatus.failed` に `retry: RetryTarget` を持たせる。`RetryTarget` は落ちた往復を送り直すのに要る値だけを持つ判別ユニオンで、`{ kind: "save" }` / `{ kind: "mode"; mode; acknowledged }` / `{ kind: "conflict"; keepLocal }` / `{ kind: "revision"; revisionId }` の 4 つ。`classifySaveFailure` は呼び出し元から受け取ってそのまま載せ、Alert の「再試行」は `retryFailed(status.retry)` で分岐する。

### Consequences

- 良い点: 失敗した操作と再試行が指す操作が型で結ばれる。`catch` を 1 つ足すと `RetryTarget` を書くことになり、その場で「これを再試行すると何が走るか」が目に入る
- 良い点: 版の復元・競合の解決が通信エラーで落ちても、書きかけを保存で上書きせずに同じ操作をやり直せる
- トレードオフ: `failed` の値が 1 つ増える。`{ ...next, stashed: true }` の書き換えはそのまま通る
- 変異スポットチェック: `retryFailed` を `commit(true)` に固定すると、版の復元の失敗からの再試行が本文の保存になる

---

## ADR-125: ED-04 の門の「了解して進む」は、面に未保存があれば正本を引き直さない

### Context

門（ED-04 の WYSIWYG 警告）を立てる経路は 3 つで、そのうち退避の復元だけが**未保存の内容を面に載せたまま**門へ入る。「了解して進む」は `applyMode("wysiwyg", true)` で、既存ノートなら必ず `reseedFromServer` → `readEditState` → `seedMode(…, latest.title, latest.html)` を通るので、いま復元したばかりの本文がサーバーの正本で丸ごと置き換わる。`clearDraft` を呼んでいないので提案は出し直され恒久的な喪失ではないが、利用者から見ると「復元した内容が了解と同時に消えて、また復元を勧められる」挙動になる。

到達経路は「HTML モードで `<style>` を書く → 通信断で退避 → 再読み込み（`<style>` を持たない保存済み本文なので WYSIWYG で開く）→ 復元」。

### Decision

「了解して進む」を `acknowledgeWysiwyg` に分け、面に未保存があるか（`dirty`）で分岐する。

- 未保存があれば正本を引き直さず、いま面が持っている写し（`takeSnapshot`）をそのまま新しい面へ載せ直す。モードのラジオが未保存の変更に確認を挟むのと同じ扱い（「未保存を面に残したまま面だけ差し替える」）
- 未保存が無ければ従来どおり `applyMode(next, true)` で引き直す。面の内容は確定値と同じなので失われるものが無く、他の利用者の更新まで取り込める

### Consequences

- 良い点: 門を立てる 3 経路のうち、未保存を抱えて入る唯一の経路で書きかけが消えなくなった
- 良い点: 分岐の条件が `dirty` 1 つなので、門を立てる経路が増えても扱いが自動的に決まる
- トレードオフ: 未保存があるあいだは他の利用者の更新を取り込まないまま WYSIWYG へ入る。次の保存が競合として検出するので、握り潰しにはならない
- 変異スポットチェック: 分岐を外して常に `applyMode(next, true)` にすると、退避の復元 → 了解の経路で復元した内容が正本で置き換わる

---

## ADR-126: 端末の既定モードへ書くのは利用者が選んだ切替だけ

### Context

`switchMode` が無条件に `writePreferredMode(next)` を呼んでいた。門の逃げ先（`surfaceModeFor` が返す強制された `html`）もこの関数を通るので、`reseedFromServer` / `resolveConflict` / 退避の復元で門が立つたびに、利用者が選んでいない `html` が端末の既定として保存される。WYSIWYG で作業中に `<style>` を持つ版を復元しただけで、以後どのノートを開いても既定が HTML になっていた。

ED-05 と `spec/pages/index.md` が端末に持つと定めているのは「**選んだ**モード」であり、門の逃げ先は選択ではない。

### Decision

`switchMode` から永続化を外し、利用者の操作に由来する 2 か所だけが書く。

- `enterMode` — モードのラジオと「保存して切り替える」。門に掛からずに実際に入るときだけ書く
- `acknowledgeWysiwyg` — 警告の「了解して進む」

`spec/pages/index.md` の ED-05 の行にも「利用者が選んだモードだけを保持し、警告の逃げ先として強制されたモードは保持しない」を明記した。

### Consequences

- 良い点: 装飾を守るための一時的な退避が、端末の恒久的な既定に化けなくなった
- 良い点: 書き込み口が 2 か所に減り、どちらも利用者の操作ハンドラの直下にある
- トレードオフ: 門に掛かって「取りやめ」を選んだときは何も書かれない。利用者はモードを選び直せる状態のままなので、次に選んだ分が書かれる
- 変異スポットチェック: `switchMode` に `writePreferredMode` を戻すと、版の復元で門が立った直後に別のノートを開いたとき HTML で開く

---

## ADR-127: 節点数を 5 つ目の資源上限にする — 二乗の移動・二乗の slug・引数上限が 1 つの枠で閉じる

### Context

ADR-113 の 4 上限（膨張・深さ・CSS の入れ子・CSS の歩数）と ADR-120 の `<template>` 包みを入れたあとも、**解析後の費用が節点数で有界でなかった**。ラウンド 008 が 3 つの実測を出した。

- **包みの逃げ道が二次を丸ごと復活させる。** `TEMPLATE_SENSITIVE`（`</template` / `<form`）に当たる入力は素の経路へ落ちる。`"<br>"` 440,000 個は包みで 364 ms、`</template>` 前置きで 16,477 ms、**`<form></form>` 前置き（13 バイト）で 82,347 ms**。3 つとも最後は `NoteHtml` の 800,000 バイトで拒まれる — 必ず失敗すると分かっている入力に 82 秒である。しかも実在の保存済み Web ページはほぼ必ず `<form>`（検索ボックス）を含むので、この逃げ道は攻撃者だけでなく本来包みが効くべき入力そのものが通る道になっていた
- **`collectHeadings` が見出し数の二次。** 同じ本文の k 個目が `a`, `a-2`, … を毎回 2 から数え直す。16,000 見出しで 7,367 ms、120,000 で 10 分以上。サニタイズ後の 1 見出しは約 23 バイトなので 800 KB の本文に 34,000 見出しが**保存でき**、以後 `applyTextNodeEdits`（スコープの UoW の内側）と `listNoteRevisions`（版 1 件につき 1 回）が毎回それを払う。1 回の保存が永続的な増幅器になっていた
- **`sanitizeNodes` の `push(...)` が素の `RangeError`。** 未許可要素の unwrap が返す配列を引数として展開するので、520 KB（`<foo>` の下に `<br>` 130,000 個）で V8 の引数上限に当たる。`RangeError` は `toSerialized()` を持たず、ポートの error contract（`SystemError` と `BusinessRuleError(NOTE_HTML_TOO_COMPLEX)` の 2 つだけ）と ADR-113 の「再帰は `RangeError` を投げない形にする」に正面から反する

3 つとも根は 1 つで、上限が**バイト数と深さ**しか縛っておらず**節点数**を縛っていないことである。

### Decision

**`HtmlProcessorLimit.maxNodes = 50_000` を 5 つ目の上限として足し、`createMeteredTreeAdapter` の既存 4 課金点で数える。**

- **値の根拠は費用である。** 素の経路に残る二次は最上位節点数の二乗で、実測は 10,000 節点 14 ms / 25,000 節点 78 ms / 50,000 節点 222 ms / 100,000 節点 795 ms / 200,000 節点 3,371 ms。50,000 は、その二乗が**包んだ経路が転送境界いっぱいの本文に既に払っている 0.25 秒**（ADR-113 / ADR-120 が正典に書いた値）と同じ水準に落ちる点である。上を採ると「13 バイトで買える停止時間」がそのまま伸びる
- **この上限だけは「到達しうるどの文書も届かない高さ」ではない。** 800,000 バイトの本文に収まる節点数の実測は、散文の段落で 11,899（67 バイト/節点）、見出しの列で 72,731、5 セルの表で 118,924、素の `<br>` で 200,003。**散文の記事に対しては 4 倍の余裕があるが、1 節点 16 バイトより密な平坦な本文は拒まれる**。これは取引であり、ADR-113 の「4 上限はどれも遠い」という書き方をこの 1 行だけ更新する必要がある
- **数えるのは節点であって呼び出しではない。** 解析器は文字の連なりを小分けに渡し、既定の tree adapter は続きを既存のテキスト節点に畳む。呼び出しを数えると木ではなく字句化の刻みを数えることになり、`<li>item text here</li>` が 2 節点ではなく 6 と勘定される。`insertText` / `insertTextBefore` は委譲の前後で親の子数を見て、**増えたときだけ**課金する
- **解析器自身の足場 3 節点（fragment の文脈要素 `template` / mock document / 解析根の `html`）は枠の外に出す**（`PARSE_SCAFFOLD_NODES`）。包みの経路はさらに包みの 1 節点を上乗せする。こうして `maxNodes` は「原文の木が持てる節点数」ちょうどになり、境界が両経路で一致する（包み: `<br>` 50,000 個が通り 50,001 個が拒まれる。素: `<form></form>` ＋ `<br>` 49,999 個が通る）
- **逃げ道の振り分けは変えない。** 包みと素の挙動同値は差分テストが支持している（無作為 39 万・全数 156 万で差分 0、語彙は許可リストではなく HTML のタグ全体）。常時 2 通を作って比較する案は、病的な入力で解析を 2 回払う
- **枠に依存させず、二乗そのものも消す。** `collectHeadings` は `base → 次に試す接尾辞` の `Map` を持ち、`taken` / `claimed` が単調増加であることを根拠に、一度通り過ぎた接尾辞を再試行しない（出す id と順序は不変）。`sanitizeNodes` は `for (const child of …) out.push(child)` に落とす
- **`inlineStylesheets` は取り込んだ CSS を `filterCss` に通す。** 取得元は第三者のオリジンで、この戻り値を後段が sanitize し直さない。予算は 1 呼び出しに 1 本。ポートの JSDoc にも「呼び手は `process` を通し直さない」と責任の所在を書いた

### Consequences

- 良い点: 実測（この worktree の Node、`process` 全体）— `<form></form>` ＋ `<br>` 440,000 個: **82,347 ms → 9 ms**。`</template>` 前置き: 16,477 ms → 9 ms。包み: 364 ms → 12 ms。`<h1>a</h1>` 16,000 個: **7,367 ms → 29〜93 ms**（拒否ではなく成功したまま）。`<foo>` ＋ `<br>` 130,000 個: `RangeError` → `BusinessRuleError(NOTE_HTML_TOO_COMPLEX)` 12 ms
- 良い点: ADR-113 の「再帰は `RangeError` を投げない」がスタックだけでなく**引数上限**についても成立する。ポートの error contract の 2 つ以外は出ない
- 良い点: 上限以下の入力は 1 バイトも変わらない。`htmlProcessor.test.ts` の既存 208 ケース（サニタイズ表全体・不動点・派生投影・他の 4 メソッド）が変更前と同一のまま通り、落ちたのは節点数の上限に掛かるようになった TC-note-826 の 1 件だけである
- トレードオフ: **1 節点 16 バイトより密な平坦な本文は保存できなくなる。** 800 KB の `<br>` の列（200,000 節点）や 4 文字セルの大きな表（118,924 節点）が該当する。散文の記事（11,899 節点）には 4 倍の余裕がある。転送境界を動かす者はこの取引も見ること
- トレードオフ: `PARSE_SCAFFOLD_NODES = 3` は parse5 の内部の形（文脈要素・mock document・解析根）に依っている。上流が変わると境界が数節点ずれる — 境界のテストがそれを捕まえる
...
## ADR-137: 規約の完了条件は識別子の列挙ではなく禁止集合の計算にする

### Context

ADR-136 で「往復をまたぐ関数は生きた値の ref だけを読む」という規約を立て、完了条件を「**`await` の後に `title` / `mode` / `body` / `baseline` を直接読む箇所がゼロ**」と定めた。grep で機械的に確認できる形にしたつもりだったが、次のラウンドで**同じクラスの欠陥が 2 件**出た（`importReferences` と `pendingMode`）。

原因は完了条件が**識別子を名指しで列挙していた**ことである。`liveRef` に載せ忘れた state は、そもそも検査の対象語に入らないので検査をすり抜ける。列挙は state を足すたび・関数を足すたびに漏れる。

同じ形の失敗は継続要求表でも起きている（例外を列挙する規則が 6 ラウンド続けて漏れ、ADR-134 で多重度の分類に書き直した）。

### Decision

規約の完了条件は、**禁止集合を計算するテスト**で表す。

`NoteEditor/__tests__/liveReads.test.ts` を `adrReference.test.ts` と同型の規約走査テストとして置き、TypeScript の AST で `editor.tsx` を読んで次を**計算**する:

- **F** = `useState` の第 1 束縛名 ∪ コンポーネント直下で F を参照して作られる派生 const
- **G** = 自分の本体に `await` を持つ関数 ∪ そこから名前呼びで到達する局所関数（到達集合も計算する）
- 主張: G の中に式位置の F の識別子が **0 件**（型ノード・プロパティ名・局所名でシャドウされたものは除外）

これにより、state を足しても関数を足しても列挙の更新が要らない。

`liveRef` に載せる範囲は「島の全 state」ではなく「往復をまたぐ関数が読む値すべて」とする。全 state を載せても保証は増えず（保証はこのテストが担う）、未使用フィールドが増えるだけである。範囲は人が列挙するのではなく、このテストが強制する。

### 検討した代替案

- **lint ルール（Biome の GritQL plugin）** — forbidden 集合が動的なので表せない
- **型で表す** — 同上
- **`await` をまたぐ関数を島の外へ出す** — 2,800 行の島の中核の書き換えになり、実質 Issue #68 の関数分割。残り 2 ラウンドで新設計を入れるのは、ラウンド010 / 011 で退けた向きと同じ

### Consequences

「規約を立てたが検査の網が狭くて漏れる」という失敗の型が、この規約については消える。同種の規約を今後立てるときも、完了条件を列挙で書かず集合の計算で書く指針になる。

テストが `typescript` を import するため、`apps/web/package.json` の devDependencies に宣言を足す（hoist で解決できてはいるが、未宣言の import は pnpm 的に不健全）。

## ADR-138: `liveReads` の定義域を明示し、その中で言い切る

### Context

ADR-137 で「規約の完了条件は禁止集合を計算するテストで表す」と決め、`liveReads.test.ts` を置いた。ラウンド014 のレビュアーが**独立した runner を作って実測**したところ、次の 4 形が違反 0 で素通りした:

- 参照渡しで後から走る局所関数（`setTimeout(later)` / `.then(later)` / `requestAnimationFrame(fnA)` → `fnB`）
- `useMemo` / `useTransition` の描画値
- props
- 局所関数経由の派生値

逆に、G の関数自身の仮引数が F の名前と衝突すると偽陽性になる。現在のツリーに実例は無いが、ADR-137 と `docs/test.md` の「計算だから足しても漏れない」という主張は**定義域の外では成り立たない**。

ADR-137 の Decision（列挙をやめて計算にする）自体は正しい。誤っていたのは計算の定義域である。

### Decision

定義域を次のように広げ、**その定義域を明示したうえで**言い切る:

- **F** = 島の仮引数（分割代入の全名）∪ 直下の全束縛 − 初期化子が関数リテラル − 「描画を跨いで同一性が変わらないフック」の閉じた表（`useRef` / `useRouter` / `useId` / `useServerFn`）。setter や `startTransition` は F に残るが、**呼び出し位置の識別子は読みに数えない**
- **G** = `await` を所有する関数 ∪ 本体に**値位置の識別子**として現れる局所関数の閉包
- **シャドウ**は `fn.body` ではなく `fn` まで上げ、関数自身の仮引数を数える（偽陽性が消える）

**表に載せるのはフックの意味論だけ**（4 つ）で、state 名を書く表ではない。表に無いフック（`useMemo` / `useOptimistic` / 自作フック）は**全束縛を捕まえた値と見なす**既定にし、漏れる側ではなく余計に赤くなる側へ倒す。

検査に**陽性ケース**を置く — 最小の島 fixture に 1〜2 行を注入して、上記 6 形が赤になることと、仮引数の名前衝突が赤にならないことを固定する。

`docs/test.md` と本 ADR の主張は「**定義域の中なら** state・関数・props を足しても検査の編集は要らない」と、定義域を書いたうえで述べる。

### 検討した代替案

- **主張だけを実態に合わせ、穴は残す**（コード変更なし）— レビュアーは独立 runner を持っており、次のラウンドでも同じ 4 形を流すのが確実。文言を限定しても「陽性ケースが無い」「4 形が素通り」は事実として残る

### Consequences

定義域を広げても現在のツリーで赤くなるのは props（`target`）の読み 2 か所だけで、他の 30 語は 1 件も当たらない（実測）。その 2 か所は `liveRef` に `target` を載せて読み替える — 既存の ref にフィールドを 1 つ足すだけで、新しい仕組みではない。

「規約を立てたが検査の網が狭くて漏れる」型は、ADR-137 で列挙をやめ、本 ADR で定義域を明示したことで二重に塞がる。今後この検査を語るときは、必ず定義域とセットで述べる。
