...
- この束縛が効く前提はアプリ全体の CSRF 姿勢と同じ: 攻撃者が被害者のブラウザーに `signUpFn` の POST を発行させられるなら確認待ち Cookie も植えられる。ここで前提になるのは「アプリ側が `FormData` 経路を書いていない」ことではない — **`FormData` の POST 経路はフレームワークが server function に対して常に開けており**（クロスオリジンの `<form>` から直接叩ける。プリフライトも起きない）、アプリの呼び出し規約が JSON POST であることは攻撃者を縛らない。したがってこの防御の前提は、AC-15 の CSRF 規律を**すべての server function 呼び出しに対する同一オリジン検証（`createCsrfMiddleware`）として実際に強制していること**であり、その middleware が外れると確認待ち Cookie を植える経路が開く。
...
- **残存条件（受容）**: 二段目が働くのは「リースが失効する」ことが前提であり、**プロセスが生き続けたまま lane 解放だけが失敗したケースは拾えない**（R4-CO-W-004）。`PRUNE_LEASE_OWNER` はプロセス単位の定数なので、そのプロセスの次の cron が `beginOrResumeKind` で自分のリースを 10 分先へ延ばし続ける（決定 3 のとおり生きたリースの lane には触れない）。`releaseLane` が一度でも失敗するとその lane は `claimed` のまま `claimLanes` から見えなくなり、プロセスが再起動して owner が変わるまで回収されない。回避策として `runCron` は解放失敗時に `workRemains = true` を立て、少なくともその起動を「未完了」として返す（`pruneExpiredAuthState.ts` の `releaseLane`）が、これは報告の正直さを保つだけで lane 自体は解放されない。恒久対策（解放失敗時の明示的なリース放棄 / 解放専用の再試行）は cron スライスで扱う。
