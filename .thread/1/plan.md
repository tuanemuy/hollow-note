...
| AC-17 | メール確認リンク → ActiveUser + サインイン状態でノート一覧へ着地（AC-01 の e2e 成立に必要な glue: verifyEmail（UC-identity-002）+ /verify-email 最小ルート。ページ描画は GET、トークン消費は POST — ADR-007）。**注記: 同一ブラウザー条件つきで充足** — セッション発行は確認要求元ブラウザーに束縛され（ADR-038、login CSRF 対策）、別端末・別ブラウザーで確認リンクを開いた場合はメール確認のみ成立し「確認済み・サインインが必要」へ倒れる | spec/scenario/account.md#AC-01 | 10 |
