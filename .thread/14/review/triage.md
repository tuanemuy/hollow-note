...
# 指摘台帳 — PR #22 レビュー R4 のトリアージ

**対象:** `.thread/14/review/review-004-general.md`（Blockers **0** / Warnings **3 件**）
**束ね後:** **3 件**
**判定:** fix **2** / wont-fix **1** / defer 0 / 既出継承 **0**

## 前提

- R4 は「fix すべき指摘は残っていない・APPROVED として閉じてよい」の判定つきで、Warning 3 件はいずれも非ブロッキング。**うち 2 件は契約側（受け入れ基準・生成指示）に誤った記述を残す**ので閉じる前に直し、1 件は直すと新しい非対称を作るので wont-fix とする。
- R1〜R3 の `wont-fix` / `defer` と Key の一致する再指摘は無く、**既出継承は 0 件**。Key は `M-103` 以降を連番で採る。
- **`spec/` と実装には 1 バイトも触らない。** 正典側（`spec/adr/052:24` / `spec/inventory/adapter.md:5`）は既に `it` 名で正しく、適合スイートの `describe` に ADP ID を持つものは実測 **0 本**。修正は `.thread/14/` の作業成果物に閉じる。
- **AC-63 のコード差分は変わらない**（**35 ファイル = 変更 32 / 削除 3 / 追加 0**、機械検査の残り **14 行**）。R4 の fix でコードに新規参入するファイルは 0。

## 台帳

| Key | 指摘 | 判定 | 理由 | 再指摘回数 |
| --- | --- | --- | --- | --- |
| M-103 `.thread/14/steps.md:366,526,661` ＋ `.thread/14/adr.md:47,57,67,1041,1057` ＋ `.thread/14/plan.md:68`（ADP ID 命名規約） | 命名規約を `spec/adr/026` に帰属させる記述と「`describe` 名で追う」の記述が作業記録側に残っている（`general:W-001`） | fix | **`steps.md:366` はヘッダー文言の生成指示**なので、手順どおり書くと `spec/adr/052` の決定 2（「`describe` はスイート名だけを名乗り、ID は持たない」）に反するヘッダーができる。正典側は既に正しい — `spec/adr/052:24` は「ケース名（`it` の第 1 引数）の先頭に ADP ID を置く」、`spec/inventory/adapter.md:5` も `it` 版（実測）、`describe` に ADP ID を持つスイートは **0 本**（実測）。**レビューが列挙した 4 か所だけを直さない** — 同じ規約を語る残り 4 か所（`adr.md:47` = ADR-002 の Context が 026 の決定として引用、`:57` = ADR-002 の決定 3 のヘッダー文言、`:1041` / `:1057` = ADR-029 が 052 の決定を「`describe` / `it` 名」と両論併記、`plan.md:68` = AC-19b が要点欄の追随先を「describe 名の主張」と書く）を残すと、`:67` だけ直した ADR-002 が同じ節の中で自己矛盾し、**R1〜R3 で最も多かった「同クラスの片側置き去り」を新しく作る**。ADR-034 の決定「文言を『ケース名（`it` の第 1 引数）の先頭に ADP ID を置く』へ改める／規約は 1 つの形を指す」がそのまま当たる範囲で一括して直す。ADR-034 自身の Context / 代替案に出る `describe` 名は**誤りの記録なので変えない**。026 への言及のうち決定 1（JSDoc だけを読んで実装したものがスイートを通る）・実行形・ADR 様式の例として引くもの（`steps.md:313,417,426,530,535,627,664` / `adr.md:62,132,170,443,459`）は正しい帰属なので触らない | 0 |
| M-104 `.thread/14/plan.md:290,319-323`（AC-55 の検査） | テスト方針の AC-55 検査が AC-55 本文（新規 DOM **11 行**）と食い違い、実行すると **8** を返して false pass する（`general:W-002`） | fix | 実測: `:320` の DOM 用の正規表現は `DOM-identity-06[012]` までしか書かれておらず、R1 で加わった `DOM-identity-063`、R2 で加わった `064` / `065` を拾わないので **8**（期待 8）を返して通る。**採る修正は正規表現の拡張ではなく、AC-55 本文が正とする ID 集合差分への一本化** — パターン列挙は新規 ID が増えるたびに書き足しが要り、書き漏らすと少ない件数を返して合格になる構造そのものが欠陥で、`DOM-identity-06[0-5]` へ直しても次の採番で同じことが起きる（M-80 / M-100 が AC-55 / AC-56 の本文を集合差分・実測基準へ寄せたのと同じ理由）。**「`-E` に統一する」「旧案の ADP 行は壊れていた」の 2 つの注記は、掛かる検査ごと無くなるので落とす**（残すと存在しない検査を説明する行になる）。**あわせて `steps.md:477` の「AC-55 の 8 / 8 / 3」も直す** — レビューの列挙には無いが同じ数値の写しで、M-88 が採った「件数と内訳の正典は plan.md の AC だけで、この文書には写さない」の形にそろえる（`steps.md` に数値を残すと次の採番でまた陳腐化する）。修正後に実際に実行して **DOM 11 / ADP 8 / UC 3・消えた ID 0 件**、並んだ ID が AC-55 の列挙と 1 対 1 であることを確認した | 0 |
| M-105 `packages/core/src/adapters/conformance/scopeCleanupAdmissionStore.ts:83`（`(ADP-common-009/010)`） | 本 PR がこの回で追加したインラインコメントが短縮連記のまま（`general:W-003`） | wont-fix | 一行理由: **参照先ケースの `it` 名（`:94` の `it("ADP-common-009/010: …")`）が base 由来の短縮なので、コメントだけ全形にすると同一ファイル内に新しい非対称ができる。** `:94` まで広げると (a) ADR-035 が定めた「そのケースに触れた回で全形化する」の外へ出て、(b) ADR-029 / ADR-035 が本 PR で確定させた**到達不能 20 本が 19 本へ動く**（`ADP-common-010` が単体 grep に当たるようになるため）ので、`.thread/14/adr.md` の 3 か所と R3 の M-90 の実測をやり直すことになる。M-93 が全形化した 5 行は**ファイル冒頭のスコープ表記**（ADR-035 が「ヘッダーコメントの範囲表記も同じ」と名指しした対象）で、特定のケースを指していない。`:83` は**特定のケース（`:94`）を指す参照**なので条件が違う。`adr.md` ADR-035 の代替案節への追記も行わない（レビューの提案は「任意」で、追記は修正の経緯の記録にあたる）。**残置そのものは既存分の全形化と同じ扱い**で、`ADP-common-009` / `010` に触れる回に `:94` ごと全形にする | 0 |

## 実地検証の結果

**3 件すべてについて実物に当たった。事実として誤っていた指摘は 0 件。** 独立に再実測して一致したもの:

- `spec/adr/052:24` と `spec/inventory/adapter.md:5` が `it` 名で書かれていること、`adapters/conformance/` の `describe` に ADP ID を持つスイートが **0 本**であること（M-103）
- テスト方針の grep 版 AC-55 検査が **8 / 8 / 3** を返す一方、AC-55 本文が正とする ID 集合差分は **11 / 8 / 3・消えた ID 0 件**を返すこと（M-104）
- `scopeCleanupAdmissionStore.ts:94` の `it` 名が base 由来で本 PR の差分に無いこと、`ADP-common-010` が到達不能 20 本の列挙に含まれること（M-105）

**レビューの列挙に無い 5 か所を追加で検出した**（いずれも M-103 / M-104 と同根なので同じ Key に含めた）: `adr.md:47` / `:57` / `:1041` / `:1057`、`plan.md:68`（M-103）、`steps.md:477`（M-104）。

## 修正の実行

単独で実行した。`spec/` とコードには触れていない。

| 順 | 対象 | Key |
| --- | --- | --- |
| 1 | `.thread/14/plan.md`（テスト方針の AC-55 検査、合否の形の一覧、AC-19b） | M-103, M-104 |
| 2 | `.thread/14/steps.md`（ステップ 11 のヘッダー文言、ステップ 20 の昇格対応表、ステップ 30、17-2 の件数） | M-103, M-104 |
| 3 | `.thread/14/adr.md`（ADR-002 の Context / 決定 3 / トレードオフ、ADR-029 の Context / 代替案） | M-103 |

**Phase 5 へ送ったもの**: なし。M-105 は ADR-035 が定めた既存分の全形化の扱いにそのまま乗るので、独立した追跡を持たない。
