...
| AC-56 | `spec/inventory/usecase.md` の `UC-identity-005/006/007` / **`UC-identity-011`（60 秒の発行間隔を `findPendingByUserAndPurpose` で測る）** / **`UC-identity-013`（再認証を求める）** / **`UC-storage-002,003`（ADR 050 の DTO 縮小。レビュー R1 の M-08）** / `UC-storage-004,013` / `UC-usage-005,007` の **11 行**の要点欄が、ステップ 4 / 24 / 25 の本文改訂に追随している（`UC-identity-011` / `013` は計画レビュー R2 coverage:P-005 で追加）。**件数は実測が正**（列挙に無い行が出たら逸脱、ではなく**実測と列挙が一致するか**で見る。AC-55 / AC-64 と同じ）。ヘッダーの「最終同期」日付が更新されている（**統合により inventory の対象は 4 ファイル → 5 ファイルに増える**。AC-19a の「4 ファイル」を上書きする） | SYNC-212,213,215,218,219,226,243 の波及 | 31 |
| AC-57 | `spec/inventory/{domain,adapter,test,frontend}.md` の既存行が Issue #2 由来の本文改訂に追随している（`DOM-storage-032〜035` / **`DOM-storage-006`** / `DOM-usage-006` / `DOM-storage-012` / **`ADP-common-008`** / **`ADP-common-025`** / **`ADP-storage-018〜021`** / `TC-identity-268` / `TC-storage-043` / `PAGE-p25-001,003`）。**`PAGE-p21-001,002` は対象外** — `spec/pages/index.md:5` が「この文書はユースケース名を書かない」と定めており、P-21 の本文に改訂が無い以上、台帳側にユースケース名（`getProfile` / `checkHandleAvailability`）を持ち込むと台帳が本文を追い越す。`grep -c getProfile spec/inventory/frontend.md` と `grep -c checkHandleAvailability spec/inventory/frontend.md` がともに 0。**ADP 行 4 群はステップ 10 の統合分**（計画レビュー R2 coverage:P-006 — AC-57 が `spec/inventory/adapter.md` を対象に挙げながら ADP ID を 1 つも列挙していなかった）。**`PAGE-p25` の participant 文言は狭めていない**（SYNC-237） | SYNC-203,204,211,223,227,235,238,240 の波及 | 9, 10, 11, 12 |
...
| AC-60 | Issue #2 由来のスコープ外 7 件（**新規 Issue 3 本 ＋ 既存 Issue 4 件へのコメント**）と、**レビューの修正ラウンドで見つかった新規 6 本**（下記スコープ節の 12〜17。**16 は R2 の V9 / M-70 で、17 は R3 の仕分けで追加**）について起票・コメントが済み、**13 の参照**（Issue 番号 / コメント URL）が本 Issue の完了コメントに列挙されている（下記スコープ節の一覧と 1 対 1）。**5. の起票は必須**（R1 の `usecase:B-004` を defer した唯一の受け皿）。6. / 7. / 9. に足した相乗り 3 件は、親項目の Issue 本文 / コメントに含まれていればよく、独立した参照を要求しない | ADR-013 / ADR-014 / ADR-015 / レビュー R1 トリアージ M-06 / R2 トリアージ M-70 | 16 |
...
**spec 本文は触らない**（実装の縮退を spec へ書き写さない — `spec/adr/046`）。ステップ 16 で起票し、参照を本 Issue の完了コメントに列挙する（AC-23 / AC-60）。**合計: 新規 Issue 13 本 ＋ 既存 Issue へのコメント 4 件**（1〜4 / 5〜7 / 12〜17 が新規、8〜11 がコメント）。6. / 7. / 9. の配下の箇条書きは**親項目に相乗りさせる論点**で、独立した Issue にはしない。
...
レビュー R3 の仕分けで新たに送った分（**新規 1 本**。上の 1〜16 とは重複しない）:

17. **新規** — 本 PR が触っていない `spec/` 本文に残る、規約どおりでない既存の記述（レビュー R3 の M-91 / M-95）。骨子は次の 2 点で、どちらも**本 PR の編集対象ファイルの外**にあるため検出範囲に入らなかった。
    - (a) `spec/usecases/note.md:847` / `:977` に「以前は〜だった」の改訂履歴が 2 か所残っている。`spec/index.md:5` / `CLAUDE.md:16` は正典に改訂履歴を置かないと定めており、レビュー R1 の M-47 が同型の 3 か所（`spec/usecases/storage.md` / `spec/testcases/storage/deleteFilesByOwner.md` / `spec/inventory/test.md`）を落としたのと同じ形。`spec/` 全域を走査してこの 2 か所以外に同型の残置は無い
    - (b) `spec/manual-tests/account.md` の `### ユースケースエラーケース対応表` に `requestPasswordReset | レート制限` の行が無い。同ユースケースのエラーケース表は `origin/main` の時点でこの行を持ち、対になる `resendVerificationEmail | 間隔制限` と `signUpWithPassword | レート制限` はどちらも対応表にある。ADR 057 の既存様式（手作業で再現できないものは `対象外` ＋ 理由 1 行）にそのまま乗るので、`パスワード手段なし` 行の直後に 1 行足す
