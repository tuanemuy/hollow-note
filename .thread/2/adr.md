...
## ADR-117: 保存の成功はサーバー truth でローカル状態を種まき直す — ただし送った値が残っている欄だけ

### Status
Accepted

### Context
P-21 の編集島はローカル状態（表示名 / 自己紹介 / ハンドル）をマウント時に `profile` prop から 1 度だけ受け、`dirty` を「ローカル状態とサーバー truth の差」で判定する。保存アクションは `updateProfile` の戻り値を捨てて `router.invalidate()` だけしていたため、サーバー側の正規化（`Handle.create` の `toLowerCase()`、`DisplayName.create` の `trim()`）で値が変わる保存では、invalidate 後の `profile` が正規化済みになる一方でローカル状態は送った値のまま残り、`dirty` が true に固定される。バーの文言は `dirty` が `saved` より優先されるので「保存しました」に到達せず「未保存の変更があります」が出続け、成功しているのに失敗したように見える。

`profile` prop をそのままローカル状態へ流し込む同期（prop 変化での上書き）は、保存の往復中に利用者が打ち替えた入力を巻き戻す。楽観的更新の巻き戻りと同じ壊れ方で、正規化のケースを直す代わりに「入力が消える」というより悪い失敗を作る。

### Decision
保存アクションで `updateProfile` の戻り値（`ProfileView`）を受け取り、成功時に欄ごとの関数更新でローカル状態を種まき直す。条件は「**送信した値が今もその欄に残っているとき**」だけで、打ち替えられた欄は触らない。`router.invalidate()` は残す — 種まき直しは編集島の中の話で、ルートが持つサーバー truth の再基底化の代わりにはならない（`AccountMenu` など同じ profile を読む他の面はこちらでしか直らない）。

判断の対象になった値を結果と対で持ち、今の入力と一致するときだけ有効にするのは ADR-102 が `SaveError` に `handle` を持たせた形と同じ。あちらは「否定を畳む条件」、こちらは「肯定を反映する条件」で、規則は 1 つ。

### Consequences
- 良い点: 正規化で値が変わる保存でも「保存しました」に到達し、`dirty` が偽に落ちる。P-21 の状態一覧の「保存済み」へ到達できる。
- 良い点: 保存中の打ち替えが巻き戻らない。往復の遅い経路ほど効く。
- トレードオフ: 打ち替えた欄は正規化前の値を持ったまま残り、次の保存まで dirty のままになる。利用者が今まさに編集している値を勝手に書き換えないほうを採った。
- ローカル状態の種まき直しは `useState` の関数更新で行う。アクションの closure が持つ値は送信時点のもので、往復後の現在値ではないため。

---

## ADR-118: ハンドルの目安は「埋まっている」だけを候補付きの状態として持つ

### Status
Accepted

### Context
`spec/scenario/account.md` の AC-07 異常系は「入力中に重複を検出して代替候補を提示する」を求め、`spec/design/pages/P21-settings-profile.html` は `field-error`「このハンドルは使われています」と候補チップを同一の状態として描く。実装は入力中の `checkHandleAvailability` が `available === false` を返しても文言だけを立て、候補チップの供給元は保存失敗（`SaveError`）だけだった。候補生成 `suggestionsFor` はクライアント純粋関数なので、供給元を増やすのにサーバーもユースケースも要らない。

一方で入力中の目安には「埋まっている」以外の落ち方がある — 予約語・不正な文字はドメインのエラーとして投げられ、通信断も同じ `catch` に来る。これらに候補を出すと、`settings-1` のような**打っても通らない**一手を勧めることになる（接尾辞は予約語判定を回避しない）。

### Decision
`HandleHint` の重複を独立したバリアント `taken` に割り、そこにだけ `suggestions` を持たせる。確認が通らなかった経路は `suggestions` を持たない `problem` のままにする。候補を持てない状態が候補を持つことは型で表現できない。

候補チップは「保存失敗の候補 → 無ければ目安の候補」の順で解決する。実際に予約が落ちた値の候補を優先するのは、欄に出る文言も同じ順で `SaveError` を先に採るためで、文言と候補が別の判断から来てちぐはぐになるのを防ぐ。

### Consequences
- 良い点: モックが同一状態として描く「重複の文言 + 候補チップ」が入力中にそろう。保存を 1 度失敗させないと候補が出ない現状の遠回りが消える。
- 良い点: 「候補は名前が埋まっていると分かったときだけ」という規則がバリアントの形になり、分岐を足しても破れない。
- トレードオフ: 候補は保存の予約を経ていないので、押した候補がさらに埋まっていることはありうる。目安であることは変わらず、確定するのは保存時の予約。
