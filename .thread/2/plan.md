...
- **新設した scope 継続タスクのポートが `spec/database/index.md#scheduled_tasks` の `priority` と `running`（リース）を持たない**（レビュー R2 / Adapter W-007）。`ScopeTaskScheduler` / `ScopeTaskQueue` の並び順は `dueAt` 昇順のみで、spec が定める `priority` 列・`(priority, due_at, kind, operation_id)` の索引・priority ごとの最低枠を確保する weighted round-robin が無いため、低 priority の大量タスクが security cleanup を飢餓させうる。`status` も `pending / failed` の 2 値で、`claimDue` はリースを取らず「scope オブジェクトは single writer」という前提にのみ依存する。本 Issue の Node ランタイムは scope task tick が 1 本なので実害は無いが、複数プロセス / 複数ワーカー配備では同じ turn が同時実行されうる。縮退の事実はポート JSDoc（`application/ports/scopeTaskScheduler.ts`）にも明記した。引き継ぎ先は **#19**（→ #11 の D1 / DO 実装の前提として、契約を実装前に確定させる）。
- **OAuth の束縛 Cookie を `state` から独立した乱数にしない**（Issue #20）。載せる値は `sha256(state)` で、`state` は認可 URL とコールバック URL の両方に載って往復するため、成立するのは login CSRF の一方向の防止までで「フローを開始したブラウザーでしか消費できない」は保証されない（未消費のコールバック URL を入手した第三者は自分のブラウザーで同じ Cookie を再現できる。PKCE は `code_verifier` がサーバー側にあるため止めない）。双方向化には `OAuthFlowState` へのフィールド追加・memory アダプター・適合スイート・`startOAuthFlow` / `completeOAuthCallback`・presentation の 2 モジュールが同時に動くため本 Issue では置かない。縮退の事実は `presentation/oauthStateBinding.ts` の JSDoc にも明記した。引き継ぎ先は **#20**。
- **`updateProfile` の旧ハンドル解放に再駆動主体を置かない**（レビュー R2 / Domain-Usecase W-001）。順序（新 key 予約 → commit → activate → 旧 key を `releasing` → `release`）自体は正しいが、`activate` 済みで `releaseActiveUniqueKey` 前にプロセスが落ちると、旧ハンドルの `active` 行が `expiresAt: null` のまま残る。`reserve` は他 operation の `active` 行を lapsed 扱いしないので、**その旧ハンドルは他の利用者が永久に取れない**（所有者本人は決定的 operationId で取り直せるが、そうする動機は無い）。新ハンドル側には `reclaim`（ADR-076）という回復経路があるのに、旧ハンドル側には対応する掃除が無い。実装で閉じる正しい形は `removeIdentity` → `identityRemovalRelease` と同じ「`identity.user.handleChanged` の購読者（`previousHandle` を鍵にした冪等なコンシューマー）へ移し、`updateProfile` は commit と同一 UoW でイベントを積むだけにする」で、本 Issue では購読者の追加がテーマ横断になるため置かない。引き継ぎ先は **#9**（`/@handle` 公開ページがハンドル失効の主たる読み側で、同じイベントの購読者を持つスライス）。
- **`identityRemovalRelease` の解放判定と `beginRelease` のあいだの TOCTOU を閉じない**。ADR-104 のガード（その `providerAccountKey` を名乗る現行 identity が無いことの確認）は Global UoW の中で判定するが、実際の解放（`beginRelease` → `release`）はその外で走る。判定のコミットと `beginRelease` のあいだに同一利用者が同じ provider account を再連携し `activate` まで通ると、`beginRelease` は所有者一致の `active` 行しか見ないため新しい claim を消し、「identity 行は残るが directory の一意性予約だけ無い」状態になる（次のサインインで同一 provider account の identity が二重に生えるか、メール不一致なら別アカウントが新規作成される。**権限昇格には至らない**）。閉じる正しい形は `beginRelease` に「行の `operationId` / `userVersion` が読んだときと同じであること」を渡すポート契約の拡張で、ポート定義・適合スイート・全アダプターに波及するため本 Issue では置かない（解放直前の読み直しは窓を縮めるだけで閉じない）。Node 単一プロセスでは窓がサブミリ秒だが、複数ワーカー / リモート DB 配備で広がる。縮退の事実は `application/identity/identityRemovalRelease.ts` の JSDoc にも明記した。

  同じ「identity 行は残るが予約だけ無い」状態には**もう 1 本の到達経路**がある: `linkOAuthIdentity` / `completeOAuthSignIn` の予約サガが commit 済み・`activateUniqueKeys` 失敗で終わると `reserved` 行が残り、10 分の TTL で lapse したあとに同じ provider account を再連携すると `reserve` が通る。両ユースケースの最終 UoW は `listByUserId` → `ensureAddable` → 無条件 `insert` で「同一 `(provider, providerAccountId)` の既存行があるか」を見ないため、identity 行が二重に生える（P-22 の重複表示と 8 件枠の消費。権限昇格には至らない）。`spec/usecases/identity.md#linkOAuthIdentity` の処理フローも最終 UoW での重複確認を要求していないので spec の取りこぼしではなく、一意性を directory が担保するという設計そのものの帰結。

  引き継ぎ先はどちらも **#21**（→ #11 / #19 の前提として、契約を実装前に確定させる）。
