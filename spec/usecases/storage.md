...
2. `UploadValidationPolicy.ensureAcceptable({ purpose: "media", ... })` を呼ぶ。SVG はここで**長さだけでなく形も**受理の条件になる（単体の 1 文書として開けること、入れ子の深さが 64 以内であること。[domains/storage.md](../domains/storage.md)）。壊れた markup をサニタイザーへ渡すと、HTML パーサーの整形要素の複製で出力が入力の二乗に膨らみ、Note の本文上限（`NOTE_CONTENT_TOO_LARGE`）や素の `RangeError` という、Storage の語彙ではない形で失敗するため
...
| アップロードされた SVG が単体の 1 文書として開けない（整形式でない・入れ子が 64 段を超える） | `BusinessRuleError(UnsupportedMimeType)` |
