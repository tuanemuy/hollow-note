...
既存Membershipのrole変更はscope-local commitを先に行い、そのeventでglobal directoryを更新する。遅延中に一覧へ古いroleが見えても、mutation直前のlocal権限確認が必ず拒否するため権限昇格にはならない。directory更新はsource version条件付きで、古いeventが新しいroleを戻さない。

**削除（除名・脱退）だけは逆向きで、event も通らない。** global の edge を先に `removing` へ倒してから scope-local commit を行い、commit のあとに同じ要求パスがその edge を in-band で settle する。`workspace.membership.removed` に購読者は無い（ロール投影も除名を投影しない）。逆向きなのは、edge が消えるまで `(userId, workspaceId)` の組が占有されたままで、除名された利用者が参加し直すことも `deleteAccount` を受理させることもできないためで、scope commit が落ちたときは同じ経路が edge を元へ戻す。
...
6. **鍵を取った直後にも削除バリアを読み、拒まれたら取った候補鍵を解放してから拒否を返す**。鍵を取るのは scope の commit の**後**なので、その 2 点の間に受理された削除は、まだ `reserved` な行の脇を通り抜ける（`release` は自分の `reserved` 行を触らないと契約が定めている。[domains/workspace.md](../domains/workspace.md) の `WorkspaceSlugReservationStore`）。素通しすると `activate` が既に終端した削除のために鍵を `active` へ上げ、`active` な予約には期限も回収する掃除も無いので**その slug はサービス全体から永久に失われる**。この要求がその鍵を名指せる最後の主体である。バリアは手順 2 の修復にも同じ位置で置く（そちらの `reserve` 前のバリアが覆うのはその手前までの窓だけである）。正常系のコストは読み 1 回
...
| 既に削除が進行中 | 進行中の `operationId` を返して受理済みとして扱う（**広告値を読めたときに限る**。下記） |
...
広告値の読みは**冪等分岐より前**にあるので、この拒否は新しい削除を開く要求だけでなく**既に受理済みの削除への再送にも掛かる**。その削除はもう要求者から何も必要としていないが、要求パスは Workspace の lifecycle を読む前に広告値を確かめるため、shard の一時障害だけで再送が `WORKSPACE_DIRECTORY_UNAVAILABLE` になる。失われる状態は無く、受理済みの削除は継続 turn が最後まで運ぶので、これは可用性の一時低下である。読みを `deleting` 判定の後ろへ移す改善は本スライスでは行わない。
...
2. **既にメンバーなら**、`activating` のまま残っている自分の edge があれば settle し、既存のロールを返す（ロールは変更しない）。この判定を招待の status より**先**に置くのが、local commit は着地したが後続の global 手順を失った join の再入口になる — その時点で招待は `accepted` なので、status で先に落とすと edge を settle できる呼び出しが 1 つも無くなり、そのメンバーの一覧からワークスペースが永久に消える。**招待を消費するのは、その招待を `accepted` にしたのがこの呼び出し者自身のときに限る**（同じ transaction で読み直して判定する）。まだ `pending` の招待は書き換えず route も `consume` せずに素通しする — 署名者と `invitation.email` を照合しない設計（WS-04）なので、そうしないと**そのワークスペースの既存メンバーがリンクを開くだけで第三者宛ての招待が消え**、招待先は恒久的に `INVITATION_NOT_FOUND` に落ちる
   - この「既にメンバー」の読みは UoW の**外**で引く助言的な値である。読みと下の transaction の間に除名が commit すると、Membership がもう無いのに参加していると答える。何も書かない枝なので除名側が取り消すものは残らない
...
5. activation claim取得後だけworkspace scope の1 transactionで `Invitation.accept` と `Membership.create` を保存する。**claim 自体を補償区間の内側に置き**、claim の応答喪失も local 失敗も同じ `abandon(operationId)` を通す — claim が commit してから応答を失うと `(userId, workspaceId)` の edge が `activating` で恒久残留し、その `operationId` はどこにも永続化されないので、利用者はそのワークスペースに二度と参加できず `deleteAccount` も受理されない（期限切れ recovery は本デプロイに存在しない）。`operationId` は試行ごとに採番する — `reserveAndClaimActivation` は同 operation の再入で `membershipId` を書き換えないので、`(userId, workspaceId)` から導出すると再試行が別 id で Membership を作り `applyRoleIfNewer` の世代照合が壊れる。プロセスが断たれた場合に残る残渣は他の補償区間と同じ残余である。claim後にaccount deletionが開始した場合、削除側はこのSagaがactive/abandonedへ収束するまでmanifest scanを待つ
