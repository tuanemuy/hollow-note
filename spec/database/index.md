...
- **同一 UoW の読み**: 書き込みと同じ unit of work の中の読みは、write-set が宣言した行の像を重ねて答える。重ねられるのは鍵で引ける行だけなので、読みの形ごとに次の 3 つが決まる。(1) **offset のページを返す列挙は自分の transaction の書き込みを観測せず、どのバックエンドでも最後にコミットされた状態から答える** — 未コミットの挿入・削除を offset の並びへ差し込めないためで、バックエンドごとの裁量にはしない（裁量にすると、同じ掃引が一方では終端し他方では周回する）。(2) **自 UoW の書き込みを数えなければ正しくない読みは集約関数で書かない** — `COUNT(*)` は保存済みの行しか数えないので、行を読んでその件数を採る（発行と同じ transaction で判定するクォータがこれにあたる）。(3) 順序付きで `LIMIT` が埋まるページの読みは、**保存済みのページに自 UoW が触れた行が載っている**とき `SystemError(DatabaseError)` になる — その行を落とすと空いた枠を埋める行が overlay の外にあり、ページを完成させられないためである。したがってこの形の読みは、触れた行が保存済みのページに載りうるなら書き込みより前に呼ぶか別の turn へ分ける。**述語の外から中へ反転させた行はそのページに載らない**（保存済みの姿は述語に合致しないので、文が返す行に含まれない）ので、その読みは通り、staged 行を重ねて答える。`trashNote` 手順 5 の `findNextPurgeDeadline` がこの形で、いま `active` → `trashed` に反転させたノートを同じ transaction の答えに含める
- **原子性**: global D1 の非集約更新は単一 SQL 文、scope 内の複数更新は `transactionSync` で行う。D1 と scope DO、または2つの scope DOを1 transactionに含めない
- **scope 検証**: scope 鍵として使っている列 — 今日は `notes.owner_type / owner_id`、`tag_assignments.scope_type / scope_id` と `_scope_identity` — は object 自身の ScopeKey と一致しなければならない。adapter は**保存（`insert`）で必ず検査する**。**複数 scope の行が 1 つの表に載るバックエンドでは、加えてその列を持つ行に触れる残りのすべての経路** — 復元（`listByNote` などの行の再構成）と、**復元でも保存でもない削除**（`deleteByNote` のような有界削除と、`delete(id, expectedVersion)` のように鍵と版で 1 行を指す削除）— でも検査する。版を照合するだけの読みは行の帰属について何も言わないので、その読みが削除を導くなら同じ 2 列を突き合わせる。削除を外すと、鍵を取り違えた配線が他 object の行を黙って消す形でだけ現れ、いちばん破壊的な経路が唯一の無検査になる。scope ごとに表そのものが分かれるバックエンド（参照ランタイムの memory がこれにあたる）では、読みが他 object の行を返しえないので保存側の検査だけで同じ性質が立つ。検査は報告であって修復ではない — 交差した行は消さず、そのままにして `DataIntegrityError` を投げる。`stored_files.owner_type / owner_id`、`storage_quotas.owner_type / owner_id`、`llm_usages.user_id`、`backup_records.user_id` は**会計上の帰属**であって scope 鍵ではないので、この検査の対象外である（`StorageOwner` は「バイトが誰の勘定に付くか」を記録し、物理 scope を上書きしない。workspace ノートの匿名エクスポート成果物は利用者に帰属しない）。物理分離そのものは `_scope_identity` の pin が担保するので、帰属列を検査しなくても他 scope の行が混ざることはない
...
`scheduled_tasks.operation_id`とpublic projection outbox event IDは必ず生成元の安定IDから決定的に導出し、保存時に乱数を採番しない。どの材料を使うかは [domains/index.md](../domains/index.md) の「継続要求」が定める多重度の4分類が決め、cursorはどの分類でも鍵に入らない。1つのoperationがNoteごとのtaskを積む「対象ごとに1本」の形では `sha256(producerKind + ":" + producerOperationId + ":" + plane + ":" + noteId + ":" + projectionRevision)` を用いる。tag delete/mergeはtag operation ID、tag renameはevent ID、rebuild/author/workspace fan-outはそれぞれの開始operation/command IDを`producerOperationId`とする。cursorを材料に含めるのはglobal outboxで運ぶ継続要求の`continuationKey`だけで、こちらはturnごとに変わる鍵である。このためtransactionの応答喪失後に同じpageを再実行しても同じPK/outbox IDへupsertされ、別Noteのtaskを1件へ潰さず、重複taskも増やさない。
...
- 参加数の読みは2本ある。`countOwnedByUser`が`role = 'owner'`の`pending` / `activating` / `active`を、`countSettledByUser`がロールを問わない`active` / `pending` / `removing`を、どちらも`limit`で打ち切って数える。後者の述語は account deletion manifest が固定する集合と同じだが、**それだけでは `deleteAccount` の受理条件にならない** — `activating` は join が claim してから settle するまでの区間なので、受理は `listActivatingByUser` と対で判定し、どちらも 0 件であることを条件にする。2 本とも受理の transaction の中で読む（[usecases/identity.md](../usecases/identity.md)）
...
| `migration_id` | text | NULL可。進行中のmoveの識別子（DB制約は置かず状態機械が守る） |
| `last_migration_id` | text | NULL可。直近に完了したswitchの`migration_id`（同上） |
...
indexes: (`created_by`, `note_id`) はmembership離脱後も残る著者表示refresh、(`scope_type`, `scope_id`, `note_id`) はworkspace表示refreshに使う。どちらも `state` を含めないのは、走査が `state <> 'reserved'` の不等値で絞るためである — `note_id` の前に不等値の列を置くと keyset の順序が索引から出なくなるので、`state` は残余述語として評価させる。物理shard後は各shardの同索引を`NoteRouteFanOutReader`がscatter-gatherし、最大32 shard・同時6接続・全体200件へmergeする。opaque cursorはgenerationと各shardのkeysetを保持する（認証はしない。[ADR 063](../adr/063-public-cursor-not-authenticated.md)）。reshard中は旧新をNoteId/routeVersionで重複排除する。`created_by` はroute予約時に固定し、Note moveでは変えない。
...
| `created_at` | integer | NOT NULL |
| `updated_at` | integer | NOT NULL |
| `terminal_at` | integer | NULL可。terminal（`completed` / `rejected`）へ移す遷移が入れる（DB制約は置かず状態機械が守る） |
| `expires_at` | integer | NULL可。terminal accountDeletionだけが入れる（同上） |
...
- **インデックス**: `stored_files_owner_idx` (`owner_type`, `owner_id`, `purpose`)、`stored_files_expires_idx` (`expires_at`) WHERE `retention = 'ephemeral'` — 期限切れの回収、`stored_files_purpose_created_idx` (`purpose`, `created_at`, `id`) — 孤児メディアの走査（keyset は `(created_at, id)` で並べ、行値比較 `(created_at, id) > (?, ?)` で再開するため、同一時刻の同着を索引で解けるよう `id` を鍵に含める）、`stored_files_note_idx` (`note_id`) WHERE `note_id IS NOT NULL` — `listByNote`（`note.purged` 後の回収・所有者付け替え）と `findArtifactByNoteAndVersion`（`note_id` + `note_version` + `purpose` + 期限内判定。1 ノートあたりの行数は少ないためインデックスは `note_id` のみで足りる）
...
- bigram索引テキストにはバイト予算（1,800,000）があり、これを越える分のトークンは索引に入らない。本文の前方から入るので頭からは引けるが、末尾しか一致点が無いキーワードは引けない。打ち切りの単位はトークンで、CJK は 2 文字ビグラム、非 CJK は空白区切りの 1 語である。したがって空白を 1 つも含まない 1 語が単独で予算を越える場合は、その語が丸ごと落ちる（1.8MB の 1 語は前方一致でも引けないため、入れても得るものが無い）。投影そのものは成功する（索引に入らないことを理由に投影を失敗させると、そのNoteは再試行を経てquarantineへ行き永久に検索へ出ないため）。予算はcontentless索引の取り消しが「入れたときと同じトークン」を再導出できることに依存するので、変えると既存の索引行が綴れなくなる
...
domain event の送信箱と、消費側の重複排除表。**両 plane に同じ形で置く。** `OutboxRepository` の実装は 1 つで、渡されたセッションの側の表に当たる。
...
| `created_at` | integer | NOT NULL |
| `attempts` | integer | NOT NULL DEFAULT 0 |
| `processed_at` | integer | NULL可。配送完了時刻 |
| `failed_at` | integer | NULL可。quarantine 時刻 |
| `next_attempt_at` | integer | NULL可 |
...
| `lease_expires_at` | integer | NULL可。由来行の `scheduled_tasks.status = 'running'` のときだけ値を持つ（この表に `status` 列は無く、NULL がそのまま `pending` を写す） |
