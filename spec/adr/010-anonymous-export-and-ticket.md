...
- `Job` を判別ユニオン化し、`requestedBy: null` は `kind: "pdfExport"` かつ `parentId: null` の場合に限って構成可能とする（型 + 不変条件 + DB の CHECK 制約の三層で強制する）。サインイン済み利用者の PDF は `requestedBy: UserId` を持ち、履歴に載りキャンセルできる
