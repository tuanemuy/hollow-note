...
ただしここで引いている「1 つの基盤の上で完結させる」は現時点の候補に基づく前提であり、確定した決定ではない（[idea.md](../idea.md) の「未確定の技術選択（候補）」）。前提が変わればこの代替案は再評価の対象になる。009 は不採用理由だけでなく決定そのもの（FTS5 を `note_search` に対応づける）も前提に乗っているため、影響の範囲は [ADR 一覧と前提依存マップ](./index.md) から引くこと。
