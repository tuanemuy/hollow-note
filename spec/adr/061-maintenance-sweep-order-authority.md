...
4. `next` が非 null のとき、その lane は claimed である。**lane を駆動する呼び出し側**（cron 経路）は、それを処理するか解放する責務を負う。単発の継続 turn は駆動側ではなく、返った lane を次の継続へ引き渡す口を持たない — position は usecase の外へ出ず、view が運ぶのは `continued` だけである。その lane はリース失効まで claimed のまま残り、後続の cron が回収する。引き渡しへ変えるのは Queue producer の配線と対で行う
...
- **残存条件（継続 turn の最終 ack が返す lane）**: 継続 turn には引き渡す口が無いので、その lane はリース失効まで claimed のまま置かれ、後続の cron がリース回収で拾う。lane は漏れないが、run の進行はその失効ぶん遅れる。塞ぐには position を継続要求の payload まで運ぶ必要があり、その配線は Queue producer と対で決める
