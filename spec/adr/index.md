...
| 060 | [恒久 claim の取り壊しは観測した claim に対する条件付きにし、失われた claim は再連携で治癒する](./060-conditional-unique-claim-teardown.md) |
| 061 | [保守スイープの表順は run のスナップショットを唯一の正本とし、ack は進めた先の position を返す](./061-maintenance-sweep-order-authority.md) |
| 062 | [run が名指す表をこの配備が掃けないときは、ack で飛ばして run を完走させる](./062-unknown-sweep-table-skip.md) |
| 063 | [公開読みモデルのカーソルは認証しない — 運ぶのは位置だけで、可視性は毎回の述語が決める](./063-public-cursor-not-authenticated.md) |
| 064 | [scope task の settle は fencing token を持たず、単一 writer と `leaseMs` の帯で守る](./064-scope-task-settle-without-fencing.md) |
| 065 | [条件付き書き込みの道具は書き込みの意味で選び、guard 敗北は読み経路が返す答えへそろえる](./065-conditional-write-tool-and-guard-loss.md) |
...
| 060 恒久 claim の条件付き取り壊し | ディレクトリの書き込みが UoW の外にあること（023）、契約の正本がポート定義で検証が共有スイート（026）、鍵の値をシンクへ出さないこと（048）、claim と identity 行を対で読むこと（038）、一意性の担保元が予約ディレクトリであること（054）、配送が at-least-once で複数ワーカーでは判定と解放の窓が広がること | 取り壊しは観測した claim に対する条件付き、条件は不透明値、失われた claim は再連携で治癒する |
| 061 保守スイープの表順の正本 | 契約の正本がポート定義で検証が共有スイート（026）、乖離は正本のある側へ倒すこと（046） | 進める権威は 1 つ、進める操作が進めた先の position を返す、command key は position を作った側が mint する |
| 062 掃けない表の skip | 表集合のスナップショットが run 単位で resume 中も動かないこと（061）、参照ランタイムが 1 つであること（025） | 掃く手段が無い表は run を止める理由にならない。同一配備内のドリフトは残存条件として残す |
| 063 カーソルを認証しない | 可視性の述語が cursor によらず毎回掛かること（009）、契約の正本がポート定義で検証が共有スイート（026）、倒す向きを正本のある側で決めること（046） | cursor は位置だけを決め、内容は決めない。capability として扱わない |
| 064 settle に fencing を持たない | 1 scope = 1 Durable Object が単一スレッドで Alarm を多重起動しないこと・Cron が scope object を全列挙しないこと（021）、継続要求が payload から鍵を復元して再駆動されること（040）、契約の正本がポート定義で検証が共有スイート（026） | 単一 writer は配備が保つ性質、`leaseMs` は配備が帯から選ぶ値。1 scope に複数 writer を置く配備は実配備の前に再訪する |
| 065 条件付き書き込みの道具と guard 敗北 | 契約の正本がポート定義で検証が共有スイート（026）、倒す向きを正本のある側で決めること（046）、共有スイートが単一スレッドのバックエンドも通すこと（024） | 道具は書き込みの意味で選ぶ。敗者の答えは直列実行の答えにそろえる。そろえられない敗北だけを契約へ足す |
