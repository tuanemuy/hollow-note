...
| TC-note-822 | updateNoteBody: パーサーが入力を 4 倍を超えて膨らませる HTML（`<table><tr>` と `<svg><desc><template><tr>` の 2 経路、60 個の `<font>` × 3,000 行）／膨張が上限に収まる同じ形 — 保存する | spec/testcases/note/updateNoteBody.md#テストケース-updatenotebody | 前者は `BusinessRuleError(NOTE_HTML_TOO_COMPLEX)`、後者は通常どおり保存される。すべてのタグが閉じ入れ子も浅い整形式な入力であり、要素名の列挙では塞げない（active formatting elements の再構成による膨張） |
