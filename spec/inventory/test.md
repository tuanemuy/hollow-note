...
| TC-note-826 | updateNoteBody: 4 上限のどれにも掛からない平坦な HTML（`<p>` を 130,000 個 = 1,950,000 バイト） — 保存する | spec/testcases/note/updateNoteBody.md#テストケース-updatenotebody | サニタイズ後が 800,000 バイトを超えるため `BusinessRuleError(ContentTooLarge)` で拒まれ、その 1 回の `process` の費用が入力長に比例した範囲に収まる（節点数の二乗にならない） |
