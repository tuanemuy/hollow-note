...
| 4 上限のどれにも掛からない平坦な HTML（`<p>` を 130,000 個 = 1,950,000 バイト） | 保存する | サニタイズ後が 800,000 バイトを超えるため `ContentTooLarge` で拒まれ、その 1 回の `process` の費用は入力長に比例した範囲に収まる（転送境界と本文上限の差は必ず失敗する入力にも同じ費用を払わせるので、節点数の二乗になる経路を残さない） | |
