...
| `format: "pdf"` で生成した直後 | 同じノートを `exportNote` で PDF ダウンロードする | 同版で残りの保持期間も十分にあるため再利用され、ジョブは登録されない。再利用側が課すのは残りの保持期間の**下限**（`expiresAt >= now + 35 分`）だけで上限はなく、EX-01 の 24 時間は保持期間の下限の約束のため、7 日残る生成物を単体エクスポートに返しても約束に反しない | |
