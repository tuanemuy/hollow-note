...
リース期間は実行体を持つジョブ（単体ジョブと batch の子。kind を問わず一律）が **15 分**、batch 親の進捗リースが **60 分**、batch 親の組み立てリースが **15 分**（キューのコンシューマーの壁時計と同じ。[ADR 015](../../adr/015-cloudflare-runtime.md)）。リーパーの実行間隔は **5 分**で、最短のリース期間（15 分）より短く保つ。
...
| 失効した `running` が 150 件ある | 実行する | `limit`（既定 100）件だけ終端され、残り 50 件は次の起動（5 分後）で回収される | |
