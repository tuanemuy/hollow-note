...
| 付与に成功した | 読み取りモデルを確認する | `tag.assigned` を受けた `projectNoteChanges` が `updateTags` を呼び、`tag_names` / `tag_names_fts` / `note_search_tags` / `tag_display_names` の 4 か所が同一バッチで更新される | |
...
| 既にタグが付いているノートにもう 1 つ付ける | 読み取りモデルを確認する | ノートのタグ集合が丸ごと入れ替わり、既存のタグと新しいタグの両方が 4 か所すべてに載る | |
