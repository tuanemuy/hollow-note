...
| 統合後 | 読み取りモデルを確認する | `tag.merged` を受けた `projectNoteChanges` が対象ノートごとに `projection.reprojectRequested` を積み、その要求を処理する側が `updateTags` を呼んで、`tag_names` / `note_search_tags` / `tag_display_names` の 3 列と FTS 索引の `tag_names_fts` 列が同一バッチで更新される | |
