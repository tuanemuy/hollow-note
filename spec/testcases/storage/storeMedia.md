...
| 入れ子が 64 段の SVG / 65 段の SVG / 1,024 段の SVG | アップロードする | 64 段は成功し、65 段と 1,024 段は `BusinessRuleError(UnsupportedMimeType)` になる（境界値。64 は方針値で、サニタイザー側の走査の深さの上限 256 段より先に効く。1,024 段は breakout tag を含まない `<g>` の入れ子なので受理判定の深さの門だけが拒む形で、門が無ければ `FileTooLarge` になる） | |
