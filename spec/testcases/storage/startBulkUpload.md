...
| 取り込み先の所有者を 1 つだけ受け取る | 混在の検査を確認する | 構造的に所有文脈が混在しないため `MIXED_OWNER_SCOPE` の検査は行わない（対象を ID の並びで受け取る `requestBulkExport` / `requestBulkNoteOperation` / `requestBackup` との違い） | |
