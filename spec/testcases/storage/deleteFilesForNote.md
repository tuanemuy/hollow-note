...
| 他のノートのメディアがある | `note.purged` を処理する | `listDeletableByNote(noteId)` に現れないため削除されない | |
...
| personal barrier の receipt と一致しない operation ID（workspace 削除由来の token を含む） | 処理する | 受理判定は personal barrier の receipt 1 本なので、一致しない token は `ConflictError("CLEANUP_OPERATION_MISMATCH")` として削除を拒否する | |
| 同じイベントを 2 回受け取る | 2 回処理する | 削除済みのファイルは `listDeletableByNote` に現れず、2 回目は `deletedCount: 0` で終わる（冪等） | |
