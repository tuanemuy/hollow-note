...
| 対象が `limit` を超える | Alarm設定を確認する | 同じscopeのtaskが直後へ再設定され、残件がなくなると次の `expiresAt` に合う | |
| 最初のephemeral fileを登録する | taskを確認する | 当該scopeだけに期限回収taskが作られ、global Cronはscopeを列挙しない | |
