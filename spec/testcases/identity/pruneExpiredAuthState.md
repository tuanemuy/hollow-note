...
| あるshardで、この配備が掃ける表の削除がすべて失敗する（掃けない表のskipは分子にも分母にも入らない） | 実行する | cursorを失わず再試行し、上限超過時はDLQと運用通知へ送る | |
...
| runの表集合が現行コードの既定順と違う（表構成を変えたデプロイをまたいでresumeする） | Cronを実行する | runのスナップショットの順に最後まで進み、順序ずれで停滞しない | |
