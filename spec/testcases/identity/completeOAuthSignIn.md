...
| 既存利用者への追加が commit 後・`activate` 前で止まり、`reserved` が TTL 失効した。この残骸を含めて identity は上限の 8 件 | 同じプロバイダーアカウントで再度サインインする | `IdentityLimitExceeded` にならず、identity は 8 件のままで、claim が `active` に復旧し、セッションが発行される | |
