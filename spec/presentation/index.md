...
| Cookie の属性 | 本文書 |
| **設定から供給される値（秘密・鍵）の一覧と供給の形** | 本文書の [`AppConfig`](#appconfig) と [秘密と鍵の供給](#秘密と鍵の供給) |
...
**値の正典は「その値が意味を持つ層」に置く**。有効期間はドメイン（`Session.ttlMs` / `AuthTokenPurpose.ttlMs`）、しきい値はドメインサービス（`LoginThrottlePolicy`）、秘密と鍵は composition root が用途ごとの鍵束として供給する（下記 [秘密と鍵の供給](#秘密と鍵の供給)）。本文書がすべての値を集める台帳になるのではない — 集めると転送手段を変えるたびにドメインの決定を書き換えることになり、この文書が存在する理由そのものと反する。`AppConfig` が持つのは「配備ごとに変わり、コードにも仕様にも書けないもの」だけである。
...
| 寿命 | `Session.expiresAt` に一致させる | サーバー側の失効（[domains/identity.md](../domains/identity.md) の `Session`）が唯一の正であり、Cookie 側を長く持たせても失効済みトークンを送るだけになる。有効期間そのもの（30 日の絶対期限）は Identity 側の決定であり本文書では決めない。View は `expiresAt` を返さないため、転送境界は同じドメイン定数（`Session.ttlMs`）から同値を再導出する（[ADR 055](../adr/055-session-expiry-derivation.md)） |
...
- **署名鍵**: composition root が[鍵束として供給する](#秘密と鍵の供給)。テーブルには置かない
...
- **署名鍵**: SharePass と同じく composition root が[鍵束として供給する](#秘密と鍵の供給)（別の鍵を使う）
...
**`AppConfig` は丸ごとクライアントへ渡る**。ウェブアプリはこれをルーターのコンテキストに載せ、SSR ペイロードへ埋めて配るため、未サインインの公開ページを含むすべての応答に全体が現れる。したがって `AppConfig` に**秘密を載せない** — 署名鍵・暗号鍵は下記「[秘密と鍵の供給](#秘密と鍵の供給)」の経路を通る。

| 項目 | 用途 | 形 |
| --- | --- | --- |
| 公開ベース URL | 絶対 URL の組み立てと SSR メタデータ | 配備の起点となる URL |
| サイト名・既定のタイトル・既定の説明・テーマ色 | ドキュメント head のメタデータ | 文字列 |
...
**署名鍵・暗号鍵は composition root が用途ごとの鍵束として供給する**（[ADR 047](../adr/047-deletion-status-ticket.md) の決定 1）。それを必要とするアダプターと presentation のヘルパーへ組み立て時に渡す。秘密がプロセスへ入る口を 1 つに保ち、クライアントへ配られる `AppConfig` とも、presentation による環境変数の直読みとも分離するためである。

| 項目 | 用途 | 形 |
| --- | --- | --- |
| SharePass の署名鍵 | 共有パスワードの通過証の署名と検証 | 単一の鍵 |
| ExportTicket の署名鍵 | 書き出しチケットの署名と検証 | 単一の鍵。SharePass とは別の鍵を使う |
| 削除 status ticket の署名鍵 | 削除の進捗チケットの署名と検証（[ADR 047](../adr/047-deletion-status-ticket.md)） | **版 → 鍵の写像と、現在の版**。未設定ならプロセス毎のランダム鍵へ落とす配備もありうる（その場合は再起動で発行済みチケットが失効し、画面がそれを説明する） |
