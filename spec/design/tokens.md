...
`--text-3xl` の上限を reference の 40px から **38px** に下げている。この構造ではページタイトルが画面の最上部（極薄バーの直下）に単独で置かれ、脇に競う要素がないため、40px だと過剰に主張する。
...
`--leading-relaxed` を reference の 1.7 から **1.75** に上げている。この構造では本文の周囲に何も置かないぶん行長が視覚的に長く感じられるので、行間で受け止める。
...
**幅は 4 段階だけ**。`--content-max: 680px` は日本語で 1 行 40 字前後に収まり、GitHub のリーダブル幅（760px 前後）よりわずかに狭い。この構造は本文の左右に何も置かないため、reference の 760px では 1 行が長く感じられる。

`--wide-max` は**カレンダー表示とタイル表示、および処理履歴の親子ジョブ表**にのみ使う例外。それ以外の画面が広い幅に逃げることを許すと、この構造の主張（中央 1 カラム）が崩れる。

`--bar-height: 56px` は reference の 64px より低い。常設ナビを持たないぶんバーに載る要素が少なく、64px だと空白が目立つ。
