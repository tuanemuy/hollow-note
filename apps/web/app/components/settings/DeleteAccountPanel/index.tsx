...
type SubmitState = Readonly<{ message: string | null }>;

const IDLE_SUBMIT_STATE: SubmitState = { message: null };
  ...
  const [submitState, submit, isSubmitting] = useActionState(
    async (
      ...
      try {
        ...
        return { message: displayError(error) };
      }
  ...
  useEffect(() => {
    ...
    const poll = async (): Promise<void> => {
      try {
        ...
        if (cancelled) return;
        // ticket の失効も一時障害も、この画面から先へ進める手立ては同じ
        // （削除自体はサーバー側で進み続ける）。ticket は捨てない —
        // 一時障害なら再読込でポーリングを再開できる。
        setPhase({ kind: "settled", message: displayError(error) });
      }
    };
    ...
    <section className={dangerPanelClass}>
      ...
      <form action={submit}>
        <div className="mb-4">
          ...
          <input
            className={`${inputClass} ${submitState.message === null ? "" : inputInvalidClass}`}
            id={confirmId}
            ...
            aria-invalid={submitState.message !== null}
...
function DeletionProgress({ phase }: { phase: Phase }) {
  if (phase.kind === "completed") {
    return (
      <section className={dangerPanelClass}>
