 ...
 * が押す先である。
 *
 * 5 つの `catch`（保存・保存後の載せ直し・モード切替・競合の解決・版の
 * 復元）が同じ `failed` を作る以上、「何が落ちたか」を値に持たないと
 * 再試行は 1 つの操作しか指せない。持たせないと、版の復元が通信エラーで
 ...
 *
 * 5 つの `catch`（保存・保存後の載せ直し・モード切替・競合の解決・版の
 * 復元）が共有するので、「何が落ちたか」は呼び出し元が
 * {@link RetryTarget} で渡す。
 *
