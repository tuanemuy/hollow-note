...
const MESSAGE_BY_CODE: Readonly<Record<string, string>> = {
  ...
  // 資源の上限（[ADR 013](spec/adr/013)）による拒否。共通文言の「入力内容を
  // 確認してもう一度お試しください」は、同じ本文を送れば必ず同じ結果になる
  // この失敗に対しては実行不能な助言になる（`NOTE_LOCKED_BY_JOB` と同じ理由）。
  // 見た目からは分からない性質なので、何を変えれば通るかを言う。
