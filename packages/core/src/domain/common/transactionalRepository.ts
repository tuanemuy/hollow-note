 ...
 * `Note` cannot be passed to a `User` repository and vice versa, so
 ...
 * (e.g. `TransactionalRepository<Note, NoteId>`), not left as the raw
 ...
 * to a `Note` repository) becomes a type error. The `string` default
